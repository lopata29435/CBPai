...
async fn llm_parse_items(st: &AppState, known: &[String], batch: &[Value]) -> Option<Vec<Value>> {
    let mut sys = String::from(
        "Ты — парсер позиций кассового чека из российского магазина. Названия сильно сокращены \
         (например 'МАРК.ПЕР.Припр.д/кур.с чесн.20г', 'АКТИБ.Биойог.с черносл.об.130г'). Расшифруй каждую позицию. \
